...
Construct a dataset that only has “Excellent” responses and contains
`year`, `state`, and average of the `data_value`. Then, make a plot.
...
#order of levels for the "response" variable
response_levels = c("Poor", "Fair", "Good", "Very good", "Excellent")
ny_df$response = factor(ny_df$response, levels = response_levels)

#Create a two-panel plot
ggplot(ny_df, aes(x = response, y = data_value, fill = response)) +
  geom_boxplot() +
